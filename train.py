...
#     try:
    ...
    for epoch in  tqdm(range(args.num_epochs)):
        ...
        print('FINAL: Epoch [{}/{}], Step [{}/{}], Loss: {:.4f}, Perplexity: {:5.4f}'
              .format(epoch, args.num_epochs, i, total_step, loss.item(), np.exp(loss.item())))
        ...
        if args.model_type == 'no_attention':
            torch.save(encoder.state_dict(), os.path.join(
                args.model_path, 'encoder_'+args.mode+'_{}.ckpt'.format(epoch+1)))
        ...
        print('FINAL: Epoch [{}/{}], Step [{}/{}], Validation Loss: {:.4f}, Validation Perplexity: {:5.4f}'
              .format(epoch, args.num_epochs, i, total_step_val, loss.item(), np.exp(loss.item())))
...
#         rouge_res = rouge(gts, res)
        obj_blue = Bleu()
        bleu_res, bleu_reses = obj_blue.compute_score(gts, res)
        print(f"Bleus: {bleu_reses}")
        
        obj_cider = Cider()
        cider_res, cider_reses = obj_cider.compute_score(gts, res)
        print(f"Ciders: {cider_reses}")
        
        obj_rouge = Rouge()
        rouge_res, rouge_reses = obj_rouge.compute_score(gts, res)
        print(f"Rouges: {rouge_reses}")
...
#     parser.add_argument('--image_dir_val', type=str, default='data/Imagetest/val', help='directory for resized images')
    parser.add_argument('--image_dir', type=str, default='data/resized_train', help='directory for resized images')
    parser.add_argument('--image_dir_val', type=str, default='data/resized_val', help='directory for resized images')
#     parser.add_argument('--predict_json', default='output_test', help='fold with output json file')
    parser.add_argument('--predict_json', default='result', help='fold with output json file')
...
#     parser.add_argument('--caption_path_val', type=str, default='data/Imagetest/val.json', help='path for train annotation json file')
    parser.add_argument('--caption_path', type=str, default='data/annotations/train.json', help='path for train annotation json file')
    parser.add_argument('--caption_path_val', type=str, default='data/annotations/val.json', help='path for train annotation json file')
